...
void display_rule(void)	//게임 규칙을 출력 
{
	gotoxy(1, 1);
	printf("보물찾기");
	gotoxy(1,3);
	...
	gotoxy(1,7);
	basic_map();	//기본 지도를 출력하는 함수 basic_map을 호출 
	printf("\n아무키나 누르면 시작합니다. \n");
	getch();
}

void basic_map(void)	//기본 지도를 출력 
{
	int i, j;
	for(i=0;i<10;i++)
	{
...
void make_treasure(int tx[], int ty[])	//보물의 위치를 결정 
{
	int i;
    for(i=0;i<2;i++)
    {
...
void move_arrow_key(char key, int *x1, int *y1, int x_b, int y_b)	//화살표 키에 따른 커서의 위치 이동 
{
	switch(key)
	{
...
void game_control(int tx[], int ty[])	//게임 제어 
{
	...
	{
		gotoxy(x, y);
		//현재 커서의 위치에 □를 출력하고 matrix[(x+1)/2][y]=1
		printf("□");
		matrix[(x+1)/2][y]=1;

		//현재 커서의 위치에 보물이 있다면 matrix[(x+1)/2][y]=2
		for(i=0;i<2;i++)
		...
		count=0;	//찾은 보물의 개수(count)를 계산하고 값을 출력 
		for(i=1;i<=10;i++)
			for(j=1;j<=10;j++)
				if (matrix[i][j]==2)
					count=count+1;
		...
		key=getch();
		move_arrow_key(key, &x, &y, 19, 10);	//화살표 키 이동, 함수 
	}while(count<2);
}
